...
def plot_hemisphere_and_global_by_year(data, time_scale, title="NH-SH-Global", 
    y_label="Solar Insolation W/m^2", fixed_ylim=False, set_x_ticks=True):
    plt.plot(time_scale, data["global"], 'k-x', label="Global")
    plt.plot(time_scale, data["nh"], 'g-x', label="NH")
    plt.plot(time_scale, data["sh"], 'b-x', label="SH")
    if(set_x_ticks):
